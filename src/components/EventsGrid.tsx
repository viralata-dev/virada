...
export function EventsGrid({ data: initialData }: EventsGridProps) {
  ...
  useEffect(() => {
    ...
    const calculateTimelineHeight = (dayEvents: Event[]) => {
      ...
      const timeValues = dayEvents.map((e) => {
        ...
        return timeMatch ? Number.parseInt(timeMatch[1], 10) : 0;
      });
    ...
    for (const [locationName, events] of Object.entries(allEventsByLocation)) {
    ...
    for (const events of Object.values(saturdayEvents)) {
    ...
    for (const [locationName, events] of Object.entries(saturdayEvents)) {
      filtered[locationName] = events;
    }

    for (const [locationName, events] of Object.entries(sundayEvents)) {
      filtered[locationName] = filtered[locationName]
        ? [...filtered[locationName], ...events]
        : events;
    }

    for (const [locationName, events] of Object.entries(otherEvents)) {
      filtered[locationName] = filtered[locationName]
        ? [...filtered[locationName], ...events]
        : events;
    }
  ...
  useEffect(() => {
    ...
    const locationsWithSortedEvents = initialData.locations.map((location) => {
            ...
            {Object.keys(filteredEvents).length > 0 ? (
              Object.entries(filteredEvents).map(([locationName, events]) => {
                ...
                const saturdayEvents = events
                  .filter((event) => event.date === "24.5")
                  .sort((a, b) => timeToHour(a.time) - timeToHour(b.time));

                const sundayEvents = events
                  .filter((event) => event.date === "25.5")
                  .sort((a, b) => timeToHour(a.time) - timeToHour(b.time));

                const otherEvents = events
                  .filter((event) => event.date !== "24.5" && event.date !== "25.5")
                  .sort((a, b) => timeToHour(a.time) - timeToHour(b.time));
                      ...
                      <ScrollArea style={{ height: "70vh" }} offsetScrollbars>
                        <Stack>
                          ...
                          {saturdayEvents.length > 0 && (
                            <>
                              <Title order={4} mt="md" style={{ color: "#1976d2" }}>
                                Saturday (24.5)
                              </Title>
                              {saturdayEvents.map((event) => (
                                <EventCard
                                  ...
                                  onToggleAttending={(attending) =>
                                    handleToggleAttending(locationName, event.artist, attending)
                                  }
                                />
                              ))}
                            </>
                          ...
                          {sundayEvents.length > 0 && (
                            <>
                              <Title order={4} mt="md" style={{ color: "#9c27b0" }}>
                                Sunday (25.5)
                              </Title>
                          ...
                          {otherEvents.length > 0 && (
                            <>
                              <Title order={4} mt="md" style={{ color: "#757575" }}>
                                Other Events
                              </Title>
          ...
          <Box style={{ display: "flex", gap: "16px", paddingBottom: "16px" }}>
            {Object.keys(filteredEvents).length > 0 ? (
              Object.entries(filteredEvents).map(([locationName, events]) => {
                ...
                // biome-ignore lint/correctness/noUnusedVariables: Used for calculating timeline heights
                const calculateTimelineHeight = (dayEvents: Event[]) => {
                  if (dayEvents.length === 0) return 0;
