import { Button, Checkbox, CheckboxGroup, Collapse, Group, MultiSelect, RangeSlider, Stack, Text } from "@mantine/core";
...
export
    ...
    function HierarchicalFilters({
        ...
        onTimePresetChange: (preset: "now" | "next" | "all" | null) => void;
        onCustomTimeRangeChange: (range: [number, number]) => void;
        onCategoriesChange: (categories: Set<string>) => void;
        onRegionsChange: (regions: Set<string>) => void;
        onVenuesChange: (venues: Set<string>) => void;
    ...
    const tagsData = facetOptions.tags.map((t) => ({
        value: t.tag,
        label: `${t.tag} (${t.count})`,
    }));
        ...
        <Stack bd="1px solid #ff6ec7" p="md" bdrs="md" pos="sticky" top={0} gap="md">
            ...
            <Collapse expanded={expanded} transitionDuration={200}>
                <Stack gap="lg">
                    ...
                    {/* TAGS FILTER - Complementary filter after venue */}
                    {tagsData.length > 0 && (
                        <Stack gap="xs" p="sm">
                            <Text fw={600} size="sm">
                                Tags
