...
function getTimelinePlacement(event: NormalizedEvent, timelineStart: Date) {
    ...
    const height = Math.max(MIN_CARD_HEIGHT, minutesToPixels(getTimelineMinutesBetween(range.start, range.end)));
...
export default function FavoritesPage() {
    ...
    const timelineBounds = useMemo(() => {
        const ranges = allFavoriteEvents
            ...
            .filter((range): range is NonNullable<ReturnType<typeof getEventTimeRange>> => range !== null);
                ...
                {favoriteEventsByRegion.length === 0 || !timelineBounds ? (
                    ...
                    >
                        <Box style={{ position: "sticky", top: 16, alignSelf: "start", zIndex: 1 }}>
                            <Box style={{ position: "relative", height: timelineBounds.height + REGION_LANE_TOP_OFFSET }}>
                                <Box
                                    style={{
                                        position: "absolute",
                                        left: "50%",
